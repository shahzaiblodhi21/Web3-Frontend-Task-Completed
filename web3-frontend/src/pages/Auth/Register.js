...
function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If the registration is successful, store the token (or handle as needed)
      ...
      // Handle error and show specific error message from the server
      if (error.response && error.response.data && error.response.data.message) {
        setSnackbarMessage(error.response.data.message); // Show backend error message
      } else {
        setSnackbarMessage('Registration failed, please try again.'); // Fallback error message
      }
            ...
            <form className='login-form' onSubmit={handleSubmit}>
              ...
              <div className="form-group" style={{ marginBottom: '35px' }}>
                <div className="d-flex align-items-center position-relative">
                  <svg className='ico' style={{ position: 'absolute', marginBottom: '11px' }} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
                    <path d="M19 21V19C19 17.9391 18.5786 16.9217 17.8284 16.1716C17.0783 15.4214 16.0609 15 15 15H9C7.93913 15 6.92172 15.4214 6.17157 16.1716C5.42143 16.9217 5 17.9391 5 19V21" stroke="#6C6C6C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path d="M12 11C14.2091 11 16 9.20914 16 7C16 4.79086 14.2091 3 12 3C9.79086 3 8 4.79086 8 7C8 9.20914 9.79086 11 12 11Z" stroke="#6C6C6C" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                  <input
                    type="text"
              ...
              <div className="mt-3 text-center" style={{ fontSize: '12px' }}>
                ...
                <a href='/' style={{ color: '#E27625', textDecoration: 'underline', marginLeft: '5px' }}>Login </a>
              </div>
            </form>
          </div>
        </div>
